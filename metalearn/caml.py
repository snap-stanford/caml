...
class CAMLAdapter():
    ...
    def __call__(self, train_loader, val_loader, model, adapt_opt,other_opt=None):
        """
            ...
            if self.args.sin_learner == True:
                ...
                try:
                    if self.args.null_patients2:
                        ...
                        task_embedding = torch.cat([task_embedding, torch.zeros_like(task_embedding)])
            ...
            if self.args.sin_learner == True:
                for _ in range(self.args.num_update_steps_global_objective):
                    ...
                    global_loss = model.global_loss(
                        prediction=global_prediction.view(-1), target=labels
                    )
                ...
                if args.caml_r_learner:
                    ...
                    loss = (loss_fn(tau_pred.view(-1), labels) * weights).mean()
                    #loss = self.loss_fn(tau_pred.view(-1), labels) * weights
                else:
                    loss = self.loss_fn(tau_pred.view(-1), labels) 
                # Regularization:
        ...
        with torch.no_grad():
            ...
            for eval_iter, batch in enumerate(tqdm(val_loader)):
                ...
                val_loss_batch = self.loss_fn(tau_pred.view(-1), labels)
                #pdb.set_trace()
